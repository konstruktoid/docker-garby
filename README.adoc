...
`dockerPrune=${dockerPrune:=no}` If you want to use the internal Docker prune option, negates `maxSecondsOld`. +
...
`networkPrune=${networkPrune:=yes}` Remove unused networks using the internal Docker prune option. +
